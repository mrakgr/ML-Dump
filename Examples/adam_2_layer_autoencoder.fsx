...
let relu_dropout_autoencoder_fine_tune weights1 weights2 learning_rate exp_decay1 exp_decay2 epsilon num_epochs (training_batches: (dM*dM) []) dtest_data keepin_rate keepin_rate2 =
    let batch,_ = training_batches.[0]
    ...
    let z4 = sgemm nT nT 1.0f weights1 z3
    ...
    let z4_c = sgemm nT nT 1.0f weights1 z3_c
    ...
    let costSquaredError batch =
        ...
        let z4 = sgemm2 nT nT 1.0f weights1 z3 0.0f z4_c
    ...
    let gradient batch = 
        let inv_batch_size = 1.0f / float32 batch.num_cols
        let z1 = sgemm2 T nT keepin_rate weights1 batch 0.0f z1
        reluActivationModule.Apply(z1, z1) |> ignore
        ...
        reluActivationModule.Apply(z3, z3) |> ignore
        let z4 = sgemm2 nT nT 1.0f weights1 z3 0.0f z4

        let squared_cost_error1 = binaryErrorModule.Apply(batch, z4, squared_cost_error1)
        //sgemm2 nT T (-inv_batch_size*learning_rate) squared_cost_error1 z3 1.0f weights1 |> ignore
        sgemm2 nT T 1.0f squared_cost_error1 z3 0.0f grad1 |> ignore
        ...
        sgemm2 nT T 1.0f squared_cost_error2 z2 0.0f grad2 |> ignore
        ...
        sgemm2 nT T 1.0f z1 squared_cost_error3 1.0f grad2 |> ignore
        ...
        //sgemm2 nT T (-inv_batch_size*learning_rate) batch squared_cost_error4 1.0f weights1 |> ignore
        sgemm2 nT T 1.0f batch squared_cost_error4 1.0f grad1 |> ignore

    let m1 = createEmptyMatrixLike weights1
    setModule.Apply(0.0f,m1,m1) |> ignore
    let m2 = createEmptyMatrixLike weights2
    setModule.Apply(0.0f,m2,m2) |> ignore

    let v1 = createEmptyMatrixLike weights1
    setModule.Apply(0.0f,v1,v1) |> ignore
    let v2 = createEmptyMatrixLike weights2
    setModule.Apply(0.0f,v2,v2) |> ignore
...
relu_dropout_autoencoder_fine_tune weights weights2 0.05f 0.9f 0.999f 1e-8f 20 training_batches dtest_data keepin1 keepin2
