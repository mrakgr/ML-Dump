...
type dMatrix = 
    struct
        val num_rows:int
        val num_cols:int
        val dArray: CudaDeviceVariable<float32>

        new (num_rows: int,num_cols,dArray: CudaDeviceVariable<float32>) = {num_rows=num_rows;num_cols=num_cols;dArray=dArray}
...
let calculateBias alpha (error: dMatrix) =
    ...
    cudnn.ConvolutionBackwardBias(alpha/float32 error.num_cols,dstTensorDesc,error.dArray,0.0f,biasTensorDesc,bias.dArray)
    bias
...
let calculateBias2 alpha (error: dMatrix) (bias: dMatrix) =
    ...
    cudnn.ConvolutionBackwardBias(alpha/float32 bias.num_cols,dstTensorDesc,error.dArray,0.0f,biasTensorDesc,bias.dArray)
    //str.Synchronize()
...
type dMatrix with
    static member createRandomUniformMatrix weights_num_rows weights_num_cols (scaling_factor : float32) location =
        let weights_total_size = weights_num_rows*weights_num_cols
        
        let cudaBuffer = new_dev<float32> weights_total_size
...
let d1 = dMatrix.createRandomUniformMatrix 15 1 1.5f 2.0f
d1.dArray |> to_host
...
and RDM = 
    ...
    static member makeUniformRandomNode(hidden_size,input_size) =
        let scale = (1.0f / sqrt(hidden_size+input_size |> float32))
