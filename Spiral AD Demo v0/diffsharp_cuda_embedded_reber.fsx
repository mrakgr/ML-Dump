// For the experimental branch by me.

// Note: The order of the referenced dll matters. Unless ManagedCuda is first, it will give errors.
...
#r @"C:\DiffSharp\src\DiffSharp\bin\Debug\DiffSharp.dll"
...
let makeUniformRandomDM(hidden_size, input_size) =
    ...
    let t = [|for x=1 to hidden_size*input_size do yield (rng.NextDouble()-0.5 |> float32)*scale|]
    let t' = new dMatrix(hidden_size, input_size, t)
    DM(t')

let makeUniformRandomDV hidden_size =
    let scale = (2.0f / sqrt(hidden_size+1 |> float32))
    let t = [|for x=1 to hidden_size do yield (rng.NextDouble()-0.5 |> float32)*scale|]
    let t' = new dMatrix(hidden_size, 1, t)
    DV(t')
...
let make_data_from_set target_length =
    ...
    let d_training_data =
        ...
        for i=0 to target_length-1 do
            ...
            let t = new dMatrix(batch_size, 7, input)
            let t' = DM t
            yield t' |> DM.Transpose|]
    ...
    let d_target_data =
        ...
        for i=1 to target_length-1 do // The targets are one less than the inputs. This has the effect of shifting them to the left.
            ...
            let t = new dMatrix(batch_size, 7, output)
            let t' = DM t
            yield t' |> DM.Transpose|]

    d_training_data, d_target_data

let train_lstm_reber num_iters learning_rate (data: DM[]) (targets: DM[]) (v_data: DM[]) (v_targets: DM[]) clip_coef (l1: LSTMLayer) (l2: Layer) =
    [|
    ...
    while i <= num_iters && System.Single.IsNaN rr = false do
        ...
        let training_costs = costs data targets
        let r = (Array.sum training_costs) / float32 training_costs.Length

        printfn "The training cost is %f at iteration %i" (float32 r.P) i

        rr <- float32 r.P

        r |> reverseProp (D 1.0f)

        let validation_costs = costs v_data v_targets
        let vr = (Array.sum validation_costs) / float32 validation_costs.Length

        printfn "The validation cost is %f at iteration %i" (float32 vr.P) i
        ...
        i <- i+1
        yield float32 r.P, float32 vr.P|]
...
let s = [|
    for i=1 to 1 do
        yield train_lstm_reber 100 5.0f d_training_data_20 d_target_data_20 d_training_data_validation d_target_data_validation 1.0f l1 l2 
        |] |> Array.concat
#time
// 3.9 for Spiral vs 12.5s for Alea AD vs 45s for DiffSharp OpenBlas.
