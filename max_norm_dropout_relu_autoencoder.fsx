...
let dropoutModule = new DeviceBinaryTransformModule<float32> <@ fun a b -> if b <= dropout_rate then a else 0.0f @>

// Max norm regularization module. It renormalizes the matrix to c if the euclidean norm is greater than c.
let maxNormModule = new maxNormRegularizationModule(784)

let relu_dropout_max_norm_autoencoder weights learning_rate norm_constant num_epochs (training_batches: (dM*dM) []) dtest_data =
    ...
    let gradient batch weights dropout_matrix learning_rate = 
        let inv_batch_size = 1.0f / float32 batch.num_cols
    ...
    for epoch=1 to num_epochs do
        let float_num_epochs = float32 num_epochs
        let learning_rate_decayed = max 0.0f (learning_rate*((float_num_epochs-(float32 epoch-1.0f))/ float_num_epochs))
        fillRandomUniformMatrix 1.0f dropout_matrix
        for batch,_ in training_batches do
            gradient batch weights dropout_matrix learning_rate_decayed
            maxNormModule.Apply(weights, weights, norm_constant) |> ignore

        printfn "Learning rate after epoch %i is %f" epoch learning_rate_decayed//(costSquaredError dtest_data weights)
...
relu_dropout_max_norm_autoencoder weights 0.01f 1.0f 20 training_batches dtest_data
