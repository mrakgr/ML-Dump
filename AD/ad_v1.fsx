...
let addb a b = // b is for bias and a is for preactivations.
    let addb va vb =
        let c = addBias va vb
        let fl out = out
        let fr out = calculateBias 1.0f out
        DMRb(DM_rec.create c,fl,fr,a,b)
    let va = a.r.P
    let vb = b.r.P
    addb va vb
...
let sigmoid (a:RDM) =
    let s va =
        ...
        let fb out = sigmoidErrorModule.A(c,out)
...
let add alpha a beta b =
    let add va vb =
        ...
        let fl out = if alpha <> 1.0f then sgeam nT nT alpha out 0.0f out else out
        let fr out = if beta <> 1.0f then sgeam nT nT 0.0f out beta out else out
        DMRb(DM_rec.create c,fl,fr,a,b)
    let va = a.r.P
    let vb = b.r.P
    add va vb
...
let square a =
    let s va =
        ...
        let fb out = squareErrorModule.A(va,out)
        DMRu(DM_rec.create c,fb,a)
    let va = a.r.P
    s va
...
let rec propagateReverseRDf (out: Rf) =
    match out with
    | DfR_Df_Df(r,fb,b) ->
        b.r.A <- fb r.A // This should also decrease the counter by one and add to the adjoint instead of setting it, but nevermind that for now.
