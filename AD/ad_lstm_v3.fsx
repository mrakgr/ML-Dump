// Turns out there were no data races and the problem was in the previous tape system. I've replaced it with the simplest one and now everything works
// quite well. This reverse AD is also quite fast as well.

// While it has the ability to reuse the tape, I have yet to implement the ability to make the reuse flexible. Currently the inputs are bound to it.
// Most likely I am going to have to make the primals refs to gain this flexibility.

// Later.

// Now that I've finally finished the LSTM, after long last and after many painful debugging sessions, I am interested in doing something else.
// The next on the agenda would be to figure out how to do more advanced AD.
...
let s = [|
        yield lstm_embedded_reber_train 200 5.0f d_training_data_20 d_target_data_20 d_training_data_validation d_target_data_validation 1.0f l1 l2 tape_t tape_v
        |] |> Array.concat
